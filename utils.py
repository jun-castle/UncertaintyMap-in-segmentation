...
def random_snippet(x, y, size=(48,48), rotate=True, flip=True):
    ...
    # get image sample
    sample = np.random.randint(0, x.shape[0])            # 1부터 20중에서

    # get x and y dimensions
    min_h = np.random.randint(0, x.shape[1]-size[0])     # 1부터 584 중에서
    max_h = min_h+size[0]                                # -> y범위 뽑는거지

    min_w = np.random.randint(0, x.shape[2]-size[1])     # 1부터 565중에서
    max_w = min_w+size[1]                                # -> x범위 뽑는거지
    
    # extract snippet
    im_x = x[sample, min_h:max_h, min_w:max_w, :]
    im_y = y[sample, min_h:max_h, min_w:max_w, :]
                                                         # 여기서 'sample': 몇번째 이미지인지
                                                         # 'min_h:max_h' : 세로 범위 & 'min_w:max_w' : 가로 범위
